...
from typing import Any, DefaultDict, List, Literal, Optional, get_args, Callable, Awaitable
...
class StrReplaceEditor(BaseTool):
    ...
    # WebSocket callback for sending editor state updates
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # Send editor state update if callback is set
        if self.ws_callback:
            try:
                # Get current file content
                current_content = await operator.read_file(path)
                ...
                state_update = {
                    ...
                    "state": {
                        ...
                        "history_count": len(self._file_history[path])
                    }
                }

                await self.ws_callback(state_update)
            except Exception as e:
                logger.error(f"Error sending editor state update: {str(e)}")
