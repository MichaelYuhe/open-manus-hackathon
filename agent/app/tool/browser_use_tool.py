        ...
        async with self.lock:
            try:
                ...
                if action == "go_to_url":
                    ...
                    result = ToolResult(output=f"Navigated to {url}")

                elif action == "go_back":
                    await context.go_back()
                    result = ToolResult(output="Navigated back")

                elif action == "refresh":
                    await context.refresh_page()
                    result = ToolResult(output="Refreshed current page")
                ...
                # Element interaction actions
                    ...
                    result = ToolResult(output=f"Clicked element at index {index}")
                    if download_path:
                        result.output += f" - Downloaded file to {download_path}"
                    ...
                    try:
                        ...
                        result = ToolResult(output=f"Scrolled to text: '{text}'")
                    except Exception as e:
                        result = ToolResult(error=f"Failed to scroll to text: {str(e)}")
                    ...
                    result = ToolResult(output=f"Sent keys: {keys}")
                    ...
                    result = ToolResult(output=f"Dropdown options: {options}")
...
"""
                ...
                # Tab management actions
                    ...
                    page = await context.get_current_page()
                    await page.wait_for_load_state()
                    result = ToolResult(output=f"Switched to tab {tab_id}")

                elif action == "open_tab":
                    if not url:
                        return ToolResult(error="URL is required for 'open_tab' action")
                    await context.create_new_tab(url)
                    result = ToolResult(output=f"Opened new tab with {url}")

                elif action == "close_tab":
                    await context.close_current_tab()
                    result = ToolResult(output="Closed current tab")
                ...
                try:
                    state_result = await self.get_current_state(context)
                    logger.info(f"Browser state: {state_result}")
                    logger.info(f"Operation info: {operation_info}")
                    if self.ws_callback and not state_result.error:
                        try:
                            state_info = json.loads(state_result.output)
                            logger.info(f"State info: {state_info}")
                            ...
                            await self.ws_callback({
                                ...
                                "state": state_info,
                                "screenshot": state_result.base64_image
                            })
                        except Exception as e:
                            logger.error(f"Error sending browser state update: {str(e)}")
                except Exception as e:
                    logger.error(f"Error getting browser state after operation: {str(e)}")
